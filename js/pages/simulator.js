    ...
    app.innerHTML = `
        <section class="max-w-4xl mx-auto px-6 pt-12">
            ...
            <div class="flex justify-center bg-gray-200 p-1.5 rounded-full mb-12 max-w-lg mx-auto">
                <button onclick="setSimMode('speed')" id="tab-speed" class="flex-1 py-2 text-xs font-semibold rounded-full bg-white text-black shadow-sm transition-all">${t.tabSpeed}</button>
                <button onclick="setSimMode('color')" id="tab-color" class="flex-1 py-2 text-xs font-semibold rounded-full text-gray-500 transition-all">${t.tabColor}</button>
                <button onclick="setSimMode('home')" id="tab-home" class="flex-1 py-2 text-xs font-semibold rounded-full text-gray-500 transition-all">${t.tabHome}</button>
            </div>
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                <div class="lg:col-span-7 bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm flex flex-col items-center">
                    <div id="simulatedWall" class="grid grid-cols-3 gap-6 p-4 bg-gray-50 rounded-3xl border border-gray-200"></div>
                    ...
                    <button id="startGameBtn" onclick="toggleGame()" class="mt-8 w-full py-3.5 rounded-full font-medium apple-btn-primary shadow-md">${t.startBtnText}</button>
                </div>
                <div class="lg:col-span-5 space-y-6">
                    <div id="homeControlPanel" class="apple-card p-6 hidden">
                        ...
                        <div class="space-y-6 text-sm">
                            <div><div class="flex justify-between mb-2"><span class="text-gray-500 font-medium">${t.homeHue}</span><span id="hueText" class="font-bold">180°</span></div><input type="range" id="hueRange" min="0" max="360" value="180" class="w-full" oninput="updateHomeColor()"></div>
                            <div><div class="flex justify-between mb-2"><span class="text-gray-500 font-medium">${t.homeSat}</span><span id="satText" class="font-bold">85%</span></div><input type="range" id="satRange" min="0" max="100" value="85" class="w-full" oninput="updateHomeColor()"></div>
                            <div><div class="flex justify-between mb-2"><span class="text-gray-500 font-medium">${t.homeBri}</span><span id="briText" class="font-bold">60%</span></div><input type="range" id="briRange" min="0" max="100" value="60" class="w-full" oninput="updateHomeColor()"></div>
                            <div><div class="flex justify-between mb-2"><span class="text-gray-500 font-medium">${t.homeTemp}</span><span id="tempText" class="font-bold">4500K</span></div><input type="range" id="tempRange" min="2700" max="6500" step="100" value="4500" class="w-full" oninput="updateHomeColor()"></div>
                        </div>
                    </div>
