window.renderHome = function(app, t) {
    app.innerHTML = `
        <section class="max-w-6xl mx-auto px-6 text-center pt-12 md:pt-20">
            <h1 class="text-5xl md:text-7xl font-bold tracking-tight text-black transition-all duration-700">${t.heroTitle}</h1>
            <p class="mt-6 text-lg md:text-xl text-gray-500 max-w-2xl mx-auto leading-relaxed">${t.heroSub}</p>
            <div class="mt-10 flex flex-col sm:flex-row justify-center items-center gap-4">
                <button onclick="navigate('simulator')" class="w-full sm:w-auto px-8 py-3.5 rounded-full font-medium apple-btn-primary shadow-lg flex items-center justify-center space-x-2">
                    <span>${t.tryBtn}</span><i data-lucide="arrow-right" class="w-4 h-4"></i>
                </button>
                <button onclick="navigate('product')" class="w-full sm:w-auto px-8 py-3.5 rounded-full font-medium apple-btn-secondary flex items-center justify-center">${t.exploreBtn}</button>
            </div>
        ...
        <section class="max-w-6xl mx-auto px-6 mt-32">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="apple-card p-8 flex flex-col justify-between">
                    <div>
                        <div class="w-12 h-12 rounded-xl bg-orange-100 flex items-center justify-center mb-6"><i data-lucide="activity" class="w-6 h-6 text-orange-500"></i></div>
                    ...
                    <div class="mt-8 flex items-center space-x-2 text-orange-500 font-semibold text-sm cursor-pointer hover:underline" onclick="navigate('simulator')">
                        <span>${t.highlightSportCTA}</span><i data-lucide="chevron-right" class="w-4 h-4"></i>
                    </div>
                </div>
                <div class="apple-card p-8 flex flex-col justify-between">
                    <div>
                        <div class="w-12 h-12 rounded-xl bg-indigo-100 flex items-center justify-center mb-6"><i data-lucide="home" class="w-6 h-6 text-indigo-500"></i></div>
                    ...
                    <div class="mt-8 flex items-center space-x-2 text-indigo-500 font-semibold text-sm cursor-pointer hover:underline" onclick="setHomeKitSimTab()">
                        <span>${t.highlightHomeCTA}</span><i data-lucide="chevron-right" class="w-4 h-4"></i>
                    </div>
                </div>
            </div>
        </section>
